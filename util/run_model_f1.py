...
import torch
import numpy as np
from util.sgd import rng, initialize, get_gradient, multithread_gradient
from util.flip_probability import flip_prob
from util.evaluate_embedding import evaluate_embedding
...
def run_model(words, dimension, root_object, train_pairs, train_siblings, val_pairs, *,
    ...
    try:
        while not finished:
            ...
            if verbose_level1:
                ...
                if last_print_time is not None:
                    print("time since last update: {:.1f}s".format(seconds() - last_print_time))
            ...
            for i in range(sub_iter_len):
                ...
                # print time
                if verbose_level1:
                    print("Negative samples: {:.2f}s".format(seconds() - t))
                # Run the SGD algorithm
                t = seconds()
                gradient = get_gradient(embeddings, dimension, train_minibatch, negative_samples, train_siblings,
                ...
                flip_time = seconds() - t
                if verbose_level1:
                    print("Times: NegativeSamples = {:.2f}s, Gradient = {:.2f}s, Flip = {:.2f}s".format(neg_sample_time, gradient_time, flip_time))

            # Evaluate the model
            # if we're doing reconstruction, use train pairs
            # if we're doing prediction, use validation pairs
            t = seconds()
            (TP, FN, FP, TN) = evaluate_embedding(embeddings,
            ...
            if verbose_level1:
            	train_template = "TRAINING:   TP = {}   FN = {}  VALIDATION:  FP = {}  (FP out of {})"
            	val_template = "VALIDATION: TP = {}   FN = {}   FP = {}  (FP out of {})"
            	print((train_template if reconstruction else val_template).format(TP, FN, FP, len(val_negatives)))
            	print("F1 score = {:.3f} • Accuracy = {:.3f}    (took {:.2f}s)".format(f1_score, accuracy, seconds() - t))
	    ...
	    '''
            ...
            if len(f1_scores) >= 2*stop_width and sum(f1_scores[-2*stop_width : -stop_width]) >= sum(f1_scores[-stop_width:]):
                finished = True
            '''
    ...
    if verbose > 0:
        ...
        #print("Last: ", f1_scores[-250:])
        print()
    
    
    print("Best Validation F1 appeared on iteration {} with F1 score {:.4f} {}".format(best_iteration, best_f1_score, str(best_tuple)))
